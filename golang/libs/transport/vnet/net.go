...
#cgo linux,box CFLAGS: -I${SRCDIR}/../../../
#cgo linux,box LDFLAGS: -L${SRCDIR}/../../../box_proj/build -lhaicam-ifaddrs -lstdc++
		...
		if err == nil {
			...
			for _, addr := range addrs {
				if addr["isIPv4"].(float64) == 1 {
					...
					mask := net.IPMask(net.ParseIP(addr["mask"].(string)))
